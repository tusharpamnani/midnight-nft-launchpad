...
export const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'contract');
const nftModulePromise = (async () => {
    ensureCompiledArtifacts();
    return import(pathToFileURL(path.join(zkConfigPath, 'contract', 'index.js')).href);
})();
export const NFTContractModule = await nftModulePromise;
const NFTContractCtor = NFTContractModule.Contract;
export function getCompiledContract(walletAddressBytes) {
    return CompiledContract.make('contract', NFTContractCtor).pipe(CompiledContract.withWitnesses({
        callerAddress: (context) => [context.privateState, walletAddressBytes]
